            ...
            *currparm = NULLC;
            ...
            for( ptr = currparm; ( tokenarray[idx].token != T_FINAL && tokenarray[idx].token != T_COMMA ) || inside_literal; idx++ ) {
                ...
                if ( inside_angle_brackets == 0 ) {
                    ...
                    DebugMsg1(("RunMacro(%s.%u): ta.strptr=%s, ta.tokpos=%s\n", macro->sym.name, parmidx, tokenarray[idx].string_ptr, tokenarray[idx].tokpos ));
                    ...
                    if ( tokenarray[idx].token == T_STRING && tokenarray[idx].string_delim == '<' && inside_angle_brackets == 0 ) {
                         ...
                         * a problem if the argument string contains '!'. See expans41.asm.
                         */
                        if ( !strchr( tokenarray[idx].string_ptr, '%' ) ) {
                            ...
                            DebugMsg1(("RunMacro(%s.%u): ta.strptr=%s, ta.strlen=%u\n", macro->sym.name, parmidx, tokenarray[idx].string_ptr, tokenarray[idx].stringlen ));
                            continue;
                        }
#endif
                        DebugMsg1(("RunMacro(%s.%u): retokenizing <> string\n", macro->sym.name, parmidx ));
                    ...
                    if ( tokenarray[idx].token == T_ID ) {
                        DebugMsg1(("RunMacro(%s.%u): T_ID token found\n", macro->sym.name, parmidx ));
