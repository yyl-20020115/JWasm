 ...
 * in:
 * CodeInfo: opnd[].InsFixup, opnd[].data32, Ofssize, prefix.adrsiz
 * CurrOpnd= index for CodeInfo.opnd[]
 *      ss = scale factor (00=1,40=2,80=4,C0=8)
...
*/
{
	...
	if ((index == EMPTY) && (base == EMPTY)) {
		...
		if ((CodeInfo->Ofssize == USE16 && CodeInfo->prefix.adrsiz == 0) ||
			...
			//if ( CodeInfo->Ofssize == USE64 && CodeInfo->prefix.RegOverride != EMPTY && SegOverride != &ModuleInfo.g.flat_grp->sym ) {
	...
	if (opndx->override != NULL) {
		if (opndx->override->token == T_REG) {
			...
			if (sym == NULL && (_stricmp(opndx->override->string_ptr, "FLAT") == 0))
				sym = (struct asym*)ModuleInfo.g.flat_grp;
#endif
		}
	...
	if (with_fixup) {
		...
		if (base == EMPTY && index == EMPTY) {
			...
			if ((!opndx->is_abs) && sym) {
				enum assume_segreg reg;
				struct asym* assume;
				reg = GetAssume(SegOverride, sym, ASSUME_DS, &assume);
				if (reg != ASSUME_NOTHING && assume)
					Ofssize = GetSymOfssize(assume);
			}

			DebugMsg1(("memory_operand: CodeInfo.Ofssize=%u, Ofssize=%u, SegOverride=%p, sym=%s\n", CodeInfo->Ofssize, Ofssize, SegOverride, sym ? sym->name : "NULL"));
			CodeInfo->prefix.adrsiz = ADDRSIZE(CodeInfo->Ofssize, Ofssize);

#if AMD64_SUPPORT
			/* v2.03: an override with a segment != FLAT will return a direct fixup */
#define Get32Fixup() (opndx->override != NULL && SegOverride != &ModuleInfo.g.flat_grp->sym) ? FIX_OFF32 : FIX_RELOFF32
...
ret_code ParseLine(struct asm_tok tokenarray[], int line_number)
/**********************************************/
{
	int                 i;
	int                 j;
	...
	if (tokenarray[0].alt_token == T_ID
		&& (tokenarray[0].token == T_DIRECTIVE 
			|| tokenarray[0].token == T_UNARY_OPERATOR
			|| tokenarray[0].token == T_BINARY_OPERATOR
			|| tokenarray[0].token == T_REG
			|| tokenarray[0].token == T_INSTRUCTION
			|| tokenarray[0].token == T_STYPE
			|| tokenarray[0].token == T_RES_ID
			)
		&& Token_Count > 1
		&& tokenarray[1].token == T_DIRECTIVE
		&& tokenarray[1].tokval == T_EQU)
	{
	...
	if (tokenarray[i].token != T_INSTRUCTION) {
		...
		if (i && tokenarray[i - 1].token == T_ID)
			i--;
		if (pi == i && (tokenarray[pi].alt_token == T_ID))
		{
			switch (tokenarray[pi].token) {
			case T_DIRECTIVE:
			case T_UNARY_OPERATOR:
				tokenarray[pi].token = tokenarray[pi].alt_token;
				return NOT_ERROR;
			}
			//
		}
