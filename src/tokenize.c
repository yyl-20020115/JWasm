...
/******************************************************************/
{
	...
	*dst++ = NULLC;
	...
	//if it is not reserved word, index = 0, it must be id
	if (index == 0) {
 ...
 */
{
	...
	for (;; ) {
		...
		//preset the alt_token to be nothing
		tokenarray[p.index].alt_token = T_NOTHING;
		tokenarray[p.index].alt_token_other = T_NOTHING;
